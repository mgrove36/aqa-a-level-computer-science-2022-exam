############################################################################
# This question refers to the PlayGame and GetChoice methods of the        #
# Breakthrough class and the creation of a new attribute (with accessor    #
# methods), PeekUsed in the Lock class.                                    #
# Introduce a (P)eek option. This can be used once per lock, and allows a  #
# player to peek into the deck to see the next three upcoming cards. There #
# should be a new command in PlayGame that only appears if the 'deck peek' #
# is still available.                                                      #
# Create a new attribute in the Lock class called PeekUsed. Create         #
# accessor methods to the Lock class to update and read the PeekUsed       #
# attribute (get/set).                                                     #
# Update the GetChoice() method in the Breakthrough class to give the user #
# the option to '(P)eek'. This menu option should only appear if the       #
# PeekUsed attribute is False.                                             #
# Introduce an option to the menu in the Play Game() method to accept 'P'  #
# as one of the menu choices. This menu option should only appear if the   #
# PeekUsed attribute is False. Display the next three cards in the deck    #
# using the GetCardDescription At() method. Set the PeekUsed attribute     #
# appropriately once the peek option has been chosen by the user.          #
# When the player is given a new lock, set the PeekUsed attribute          #
# appropriately to allow the user to use the peek option again.            #
#                                                                          #
# Test the changes you have made:                                          #
#   Run the game and peek (peek is an option, it works and then it's no    #
#   longer an option), peek again to make sure it doesn't work even though #
#   the option isn't displayed. Solve a lock and check that peek is now an #
#   option again.                                                          #
############################################################################
...
class Breakthrough():
        ...
        if len(self.__Locks) > 0:
            ...
            while not self.__GameOver:
                ...
                while not self.__LockSolved and not self.__GameOver:
                    ...
                    # Change made here #
                    ####################
                    elif not self.__CurrentLock.GetPeekUsed() and MenuChoice == "P":
                        LineOfDashes = "------------------"
                        print(f"\nTop 3 cards in deck:\n\n{LineOfDashes}")
                        for i in range(min(self.__Deck.GetNumberOfCards(), 3)):
                            print(f"| {self.__Deck.GetCardDescriptionAt(i)} ",end="")
                        print(f"|\n{LineOfDashes}\n")
                        
                        self.__CurrentLock.SetPeekUsed(True)
                    ####################
    ...
    def __GetChoice(self):
        ...
        # Change made here #
        ####################
        Choice = input(f"(D)iscard inspect, (U)se card{'' if self.__CurrentLock.GetPeekUsed() else ', (P)eek'}:> ").upper()
        ####################
...
class Lock():
    ...
    # Change made here #
    ####################
    def SetPeekUsed(self, Value):
        self._PeekUsed = Value
